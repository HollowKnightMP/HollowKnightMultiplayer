...
public static class PhotonNetwork
{
    ...
    static PhotonNetwork()
    {
...
#endif
        ServerSettings photonSettings = ScriptableObject.CreateInstance<ServerSettings>();

        photonSettings.HostType = ServerSettings.HostingOption.OfflineMode;
        photonSettings.Protocol = ConnectionProtocol.Udp;

        PhotonNetwork.PhotonServerSettings = photonSettings;


        Debug.Log("Setting if app should run in background");
		if (PhotonServerSettings != null)
		{
			Application.runInBackground = PhotonServerSettings.RunInBackground;
		}

        Debug.Log("Creating hidden photon monobehaviour");
        ...
        Debug.Log("Setting up networkingpeer");
    ...
    public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
    {
        ...
        {
            Debug.Log("Adding new PhotonView to instantiated object");

            prefabGo.AddComponent<PhotonView>();
        }
