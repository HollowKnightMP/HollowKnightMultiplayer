...
namespace HollowKnightMP.Debugging
{
    public class SceneDebugger : BaseDebugger
    {
        ...
        public SceneDebugger() : base(600, 600, null, KeyCode.S, true, false, false, GUISkinCreationOptions.DERIVEDCOPY)
        {
            ActiveTab = AddTab("Scene", RenderTabScenes);
        ...
        public override void Update()
        {
            if (Input.GetKeyDown(RayCastKey))
            {
                ...
                else
                {
                    ...
                    RaycastHit2D[] hits = Physics2D.RaycastAll(point, Vector2.zero, float.MaxValue);
        ...
        private void RenderTabGameObject()
        {
            using (new GUILayout.VerticalScope("Box"))
            {
                        ...
                        foreach (MonoBehaviour behaviour in selectedObject.GetComponents<MonoBehaviour>())
                        {
                            using (new GUILayout.HorizontalScope("Box"))
                            {
                                if (GUILayout.Button(behaviour.GetType().Name))
                                {
                                    ...
                                    ActiveTab = GetTab("MonoBehaviour");
                                }
                            }
                        }
                    }
                ...
                {
                    GUILayout.Label($"No selected GameObject\nClick on an object in '{GetTab("Hierarchy").Name}'", "fillMessage");
                }
            }
        }
        ...
        private void RenderTabMonoBehaviour()
        {
            using (new GUILayout.VerticalScope("Box"))
            {
                ...
                {
                    GUILayout.Label($"No selected MonoBehaviour\nClick on an object in '{GetTab("MonoBehaviour").Name}'", "fillMessage");
                }
            }
        }
                        ...
                        using (new GUILayout.HorizontalScope())
                        {
                            ...
                            if (!method.GetParameters().Any())
                            {
                                if (GUILayout.Button("Invoke", GUILayout.MaxWidth(150)))
                                {
                                    ...
                                    if (result != null)
                                    {
                                        Console.WriteLine($"Invoked method {method.Name} which returned result: '{result}'.");
                                    }
                                    else
                                    {
                                        Console.WriteLine($"Invoked method {method.Name}. Return value was NULL.");
                                    }
                                }
                            }
                        }
