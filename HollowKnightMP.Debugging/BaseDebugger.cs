...
namespace HollowKnightMP.Debugging
{
    public abstract class BaseDebugger
    {
        ...
        protected BaseDebugger(int desiredWidth, int desiredHeight, string debuggerName = null, KeyCode hotkey = KeyCode.None, bool control = false, bool shift = false, bool alt = false, GUISkinCreationOptions skinOptions = GUISkinCreationOptions.DEFAULT)
        {
            if (desiredWidth < 200)
            {
                desiredWidth = 200;
            ...
            WindowRect = new Rect(Screen.width / 2 - (desiredWidth / 2), Screen.height / 2 - (desiredHeight / 2), desiredWidth, desiredHeight); // Default position in center of screen.
            Hotkey = hotkey;
        ...
        public DebuggerTab GetTab(string name)
        {
            DebuggerTab tab;
